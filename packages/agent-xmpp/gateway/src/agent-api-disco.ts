 ...
 */
import type { AgentApiManifest, AgentXmppNamespaces, RegisteredAgent, RegisteredOperation } from '@agent-xmpp/protocol';
...
export function buildAgentDirectory(
  ...
  return resultIq(
    ...
    xml(
      'query',
      { xmlns: DISCO_ITEMS_NS, node: namespaces.directory },
      ...agents.map((agent) =>
        xml('item', {
          jid: agent.manifest.agent.jid,
          node: namespaces.endpoint,
...
export function buildAgentInfo(
  ...
  return resultIq(
    ...
    xml(
      ...
      ...features(namespaces.endpoint, namespaces.api, namespaces.task, VCARD_TEMP_NS, ...STANDARD_FEATURES),
...
export function buildOperationItems(
  ...
  return resultIq(
    ...
    xml(
      'query',
      { xmlns: DISCO_ITEMS_NS, node: namespaces.api },
      ...agent.operations.map((operation) =>
        xml('item', {
          jid: agent.manifest.agent.jid,
          node: operationNode(operation.name, namespaces),
...
export function buildOperationInfo(
  request: Element,
  agent: RegisteredAgent,
  ...
  return resultIq(
    ...
    xml(
      'query',
      { xmlns: DISCO_INFO_NS, node: operationNode(operation.name, namespaces) },
      xml('identity', { category: 'automation', type: 'mcp-tool', name: operation.title ?? operation.name }),
      ...features(namespaces.operation, namespaces.task),
...
export function operationNode(name: string, namespaces: AgentXmppNamespaces = DEFAULT_PROTOCOL_NAMESPACES): string {
  return `${namespaces.operation}#${encodeURIComponent(name)}`;
}

export function operationFromNode(
  node: string,
  namespaces: AgentXmppNamespaces = DEFAULT_PROTOCOL_NAMESPACES,
): string | null {
  const prefix = `${namespaces.operation}#`;
