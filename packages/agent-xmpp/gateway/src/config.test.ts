...
const PROTOCOL_KEYS = ['AGENT_XMPP_NAMESPACE_ROOT', 'AGENT_XMPP_MEDIA_VENDOR'];
...
describe('loadConfig receipt knobs', () => {
  ...
  beforeEach(() => {
    ...
    for (const k of [...RECEIPT_KEYS, ...CONNECTION_KEYS, ...PROTOCOL_KEYS]) delete process.env[k];
  ...
  it('defaults to observe-only (no resend) with sane intervals', () => {
    ...
    expect(c.protocolNamespaces?.api).toBe('urn:solstice:agent-api:1');
  });

  it('loads a custom protocol profile only at the composition root', () => {
    process.env.AGENT_XMPP_NAMESPACE_ROOT = 'urn:example';
    process.env.AGENT_XMPP_MEDIA_VENDOR = 'example';
    const c = loadConfig();
    expect(c.protocolNamespaces?.task).toBe('urn:example:agent-task:1');
    expect(c.protocolNamespaces?.taskMediaType).toBe('application/vnd.example.agent-task+json');
  });
