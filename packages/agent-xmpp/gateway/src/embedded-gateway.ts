import { DEFAULT_PROTOCOL_NAMESPACES, type AgentTaskRecord, type OutboundDeliverRequest } from '@agent-xmpp/protocol';
...
export class EmbeddedXmppGateway {
  ...
  async deliverTask(task: AgentTaskRecord): Promise<string> {
    return this.sendTracked(buildTaskInvocation(task, this.config.protocolNamespaces ?? DEFAULT_PROTOCOL_NAMESPACES));
  }
