...
describe('agent task stanza codec', () => {
  it('round-trips the durable invocation contract', () => {
    const stanza = buildTaskInvocation({
      ...
      targetJid: 'target@agents.test',
      tenantId: 'acme',
      workspaceId: 'engineering',
      endpointId: 'xmpp+mcp://target@agents.test',
      operation: 'review',
      apiVersion: '1.0.0',
      inputSchemaDigest: 'sha-256:input',
      outputSchemaDigest: 'sha-256:output',
  ...
  it('round-trips tasks under an injected namespace profile', () => {
    ...
    const stanza = buildTaskEvent(event, namespaces);
    expect(stanza.getChild('event', 'urn:example:agent-task:1')).toBeDefined();
    expect(parseTaskEvent(stanza, namespaces)).toEqual(event);
    expect(parseTaskEvent(stanza)).toBeNull();
  });
});
