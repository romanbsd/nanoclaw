...
const AGENTS: DemoAgent[] = [
  ...
  },
  {
    ...
    instructions:
      ...
      'When an agent-task asks you to run conversation.respond, do not discover or call other agents. Read Task arguments.message, ' +
      'then call task.complete with the supplied taskId and result {"response":"MIKE_REMOTE_OK"} when that exact marker is requested.',
  },
];
...
async function assertRapidMlxModel(): Promise<void> {
  const response = await fetch(`${RAPID_MLX_URL}/v1/models`);
  if (!response.ok) throw new Error(`Rapid-MLX model discovery failed: ${response.status}`);
  const payload = (await response.json()) as { data?: Array<{ id?: string }> };
  const ids = (payload.data ?? []).map((model) => model.id ?? '').filter(Boolean);
  const expected = [RAPID_MLX_MODEL, RAPID_MLX_MODEL_ID];
  assert.ok(
    ids.some((id) =>
      expected.some((candidate) => id === candidate || id.endsWith(candidate) || candidate.endsWith(id)),
    ),
    `Rapid-MLX does not advertise the selected model (${expected.join(' / ')}); found: ${ids.join(', ')}`,
  );
}
...
async function verifyJaneMikeJane(client: XmppSession, jane: ProvisionedAgent, mike: ProvisionedAgent): Promise<void> {
  ...
  log(`verified Jane → Mike → Jane remote tool round-trip: ${finalReply}`);
}

function verifyProvisionedRuntimeConfig(): void {
  const db = new Database(path.join(DATA_DIR, 'v2.db'), { readonly: true });
  try {
    for (const agent of provisioned) {
      const config = db
        .prepare('SELECT provider, model FROM container_configs WHERE agent_group_id = ?')
        .get(agent.agentGroupId) as { provider: string | null; model: string | null } | undefined;
      assert.deepEqual(config, { provider: 'opencode', model: CONTAINER_MODEL });
    }
  } finally {
    db.close();
  }
...
async function subscribeToAgent(client: XmppSession, agent: ProvisionedAgent): Promise<void> {
  const accepted = client.waitForStanza(
    (stanza) =>
      stanza.is('presence') &&
      stanza.attrs.type === 'subscribed' &&
      String(stanza.attrs.from || '').split('/')[0] === agent.jid,
  );
  const available = client.waitForStanza(
    (stanza) =>
      stanza.is('presence') && !stanza.attrs.type && String(stanza.attrs.from || '').split('/')[0] === agent.jid,
  );
...
async function main(): Promise<void> {
  ...
  assert.ok(listProviderContainerConfigNames().includes('opencode'), 'OpenCode provider is not installed');
  ...
  orchestratorProcess = spawnLogged(
    'orchestrator',
    process.execPath,
    [TSX, 'src/modules/xmpp-agent-gateway/orchestrator-server.ts'],
    sharedEnv,
  );
  ...
  verifyProvisionedRuntimeConfig();
  ...
  try {
    ...
    await verifyJaneMikeJane(human, provisioned[0], provisioned[1]);
  } finally {
