...
describe('nanoclawInboundFromBridge', () => {
  ...
  it('treats generic task envelopes as structured chat, not durable task lifecycle work', () => {
    const inbound = nanoclawInboundFromBridge(payload);
    expect(inbound.kind).toBe('chat');
    expect(inbound.content.text).toContain('Summarize XEP-0432');
    expect(inbound.content).not.toHaveProperty('prompt');
    expect(inbound.content).not.toHaveProperty('task');
  });
