...
export class XmppAgentGatewayStore {
  constructor(private readonly namespaces: AgentXmppNamespaces = DEFAULT_PROTOCOL_NAMESPACES) {}

  registerManifest(value: unknown, tenantId: string): RegisteredAgent {
    const manifest = validateManifest(value, this.namespaces.api);
  ...
  discover(tenantId: string, query: string, limit = 10): VirtualMcpEndpoint[] {
    ...
    return this.listAgents(tenantId)
      ...
      .map(({ agent }) => endpointDescriptor(agent, this.namespaces));
  }
