...
import { ensureXmppPeerDestination } from './peer-destination.js';
...
describe('ensureXmppPeerDestination', () => {
  ...
  it('registers a human peer JID as a named destination', async () => {
    createAgentGroup({
      id: 'ag-spark',
      name: 'Spark',
      folder: 'spark',
    ...
    await ensureXmppPeerDestination('ag-spark', 'john@example.org');
    writeDestinations('ag-spark', 'sess-spark');

    const db = openInboundDb('ag-spark', 'sess-spark');
    try {
      expect(db.prepare('SELECT name, platform_id FROM destinations ORDER BY name').all()).toEqual([
        { name: 'john', platform_id: 'john@example.org' },
      ]);
    } finally {
      db.close();
    }
