...
| Document | Role |
|---|---|
| [a2a.md](../a2a.md) | A2A v1.0 specification (operations + data model) |
...
| [docs/xmpp_agent_gateway_v2.md](./xmpp_agent_gateway_v2.md) | Gateway architecture |
...
All A2A objects from [a2a.md §4](../a2a.md) MUST round-trip through the representations below.
...
or exactly one of: `message`, `statusUpdate`, `artifactUpdate` — matching A2A `StreamResponse` (a2a.md §3.2.3).
...
3. Gateway wakes agent runtime if needed (see [xmpp_agent_gateway_v2.md §9](./xmpp_agent_gateway_v2.md)).
...
```json
{"statusUpdate":{"taskId":"t1","contextId":"ctx-1","status":{"state":"TASK_STATE_WORKING","timestamp":"…"}}}
```

```json
{"artifactUpdate":{"taskId":"t1","contextId":"ctx-1","artifact":{"artifactId":"a1","parts":[{"url":"https://…/report.pdf","mediaType":"application/pdf"}]},"lastChunk":true}}
```
