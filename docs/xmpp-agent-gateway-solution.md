...
| Inbound delivery         | `src/agent-inbound/`                                                            | Canonical host-to-agent session DB write and container wake path                                                   |
...
### 5.1 Stable plugin-owned XMPP identity

Migration 101 moves XMPP identity out of the core agent-group schema into the plugin-owned `xmpp_agent_identities` table. The row references its NanoClaw agent group while keeping channel-specific identity data out of the core entity.

This provides a direct mapping:

```text
bare XMPP JID -> xmpp_agent_identities.agent_group_id -> agent group -> sessions and container
```
...
The shared XMPP adapter resolves the recipient JID and places the trusted IDs in the inbound event's generic `target`. The normal router then:

1. resolves the target agent group and messaging group;
2. confirms that the inbox is wired to the resolved agent group;
3. applies the normal NanoClaw access gate;
4. resolves the agent's session; and
5. writes the message through the canonical inbound delivery path.

Unknown logical-agent JIDs are dropped and logged. They are never routed to a default local agent.

### 5.3 Canonical host-to-agent delivery

Both ordinary channel routing and remote-task delivery call `deliverAgentInbound()`. There is one runtime path because NanoClaw's session databases are the transport boundary, not a configurable transport plugin.

The implementation performs the existing NanoClaw behavior:

- insert the message into the session's `inbound.db`;
- update reply routing when requested by the caller;
- let the XMPP plugin project discovered peers into the generic destination table;
...
The input type remains narrow and reusable, but no unused registry or alternate transport abstraction is carried in core.
...
| Component `disco#items`, node `urn:solstice:agent-directory:1` | Tenant-visible logical-agent JIDs                                                            |
| Agent `disco#info`, node `urn:solstice:mcp-endpoint:1`         | Canonical endpoint ID, title, version, manifest digest, availability, and cold-start support |
| Agent `disco#items`, node `urn:solstice:agent-api:1`           | Operation nodes                                                                              |
...
The `urn:solstice:*` nodes are project extension namespaces carried over standard XMPP discovery; they are not registered XSF protocols.
...
```text
urn:solstice:agent-directory:1
urn:solstice:agent-api:1
urn:solstice:agent-operation:1
urn:solstice:mcp-endpoint:1
urn:solstice:agent-task:1
```
...
2. **Session DB is the only runtime transport.** Channel and task ingress share the same canonical delivery function; no alternate transport registry is shipped.
3. **Custom task protocol.** Remote method calls use the `urn:solstice:agent-task:1` extension, not a standardized XMPP RPC XEP.
...
The principal switch points are `GatewayRuntimeMailbox` for embedding the XMPP package, channel registry contributions for container configuration and address normalization, and the orchestrator's `OpenfireClient` for server-specific identity operations.
...
Rapid-MLX versions before v0.10.12 have a Gemma 4 tool-parser defect that affects the nested `arguments` object used by `agents.call_tool`: the non-balanced matcher stops at the first nested `}`, which truncates the call before sibling fields. The model may appear to select `agents.call_tool`, but NanoClaw cannot create the target task because its arguments are malformed. The fix was merged in [Rapid-MLX PR #1102](https://github.com/raullenchai/Rapid-MLX/pull/1102) and is scheduled for v0.10.12. NanoClaw keeps the correct structured Agent API instead of adding a provider-specific flattened argument format.
