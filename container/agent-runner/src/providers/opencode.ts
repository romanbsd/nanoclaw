...
function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  const providerOptions: Record<string, unknown> =
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            ...(providerNpm ? { npm: providerNpm, name: providerName } : {}),
            options: { apiKey: providerApiKey, baseURL: proxyUrl },
              ...
              ? {
                  models: Object.fromEntries(
                    modelsToRegister.map((mid) => [
                      ...
                      providerNpm
                        ? {
                            ...(modelContextLimit > 0 && modelOutputLimit > 0
                              ? { limit: { context: modelContextLimit, output: modelOutputLimit } }
                              : {}),
                          }
                        : { id: mid, name: mid, tool_call: true },
                    ]),
                  ),
                }
              : {}),
  ...
  return {
    ...
    ...(agentName && agentPrompt
      ? {
          ...
          agent: { [agentName]: { mode: 'primary', prompt: agentPrompt } },
        }
      : {}),
    instructions,
    mcp,
...
export function extractLatestAssistantText(messages: OpenCodeMessage[] | undefined): string | null {
...
export function latestAssistantFailed(messages: OpenCodeMessage[] | undefined): boolean {
  return (
    [...(messages ?? [])].reverse().find((message) => message.info?.role === 'assistant')?.info?.finish === 'error'
  );
}
  ...
  query(input: QueryInput): AgentQuery {
    ...
    async function* gen(): AsyncGenerator<ProviderEvent> {
      ...
      while (!aborted) {
        ...
        if (!sessionId) {
          // A pre-set title prevents OpenCode from launching a second LLM request solely to name
          // the session. This matters for local servers configured for one concurrent request.
          const created = await client.session.create({ body: { title: 'NanoClaw session' } });
          if (created.error) {
        ...
        const promptRes = await client.session.promptAsync({
          ...
          body: {
            ...
            ...(process.env.OPENCODE_AGENT_NAME ? { agent: process.env.OPENCODE_AGENT_NAME } : {}),
            ...(process.env.OPENCODE_DISABLE_BUILTIN_TOOLS === '1'
              ? { tools: Object.fromEntries(OPENCODE_BUILTIN_TOOLS.map((tool) => [tool, false])) }
              : {}),
          },
        });
        if (promptRes.error) {
        ...
        let lastRetryMessage: string | undefined;
          ...
          turn: while (true) {
            ...
            switch (ev.type) {
              ...
              case 'session.status': {
                ...
                if (st?.type === 'retry' && st.message) lastRetryMessage = st.message;
                if (
                  st?.type === 'retry' &&
        ...
        for (const [msgId, role] of roleByMessageId) {
          if (role === 'assistant') {
            eventResultText = partTextByMessageId.get(msgId) ?? eventResultText;
          }
        }
        const messages = await client.session.messages({ path: { id: sessionId } });
        if (messages.error) {
          throw new Error(`OpenCode: failed to read completed response: ${JSON.stringify(messages.error)}`);
        }
        const resultText = extractLatestAssistantText(messages.data) ?? (eventResultText.trim() || null);
        if (!resultText && latestAssistantFailed(messages.data)) {
          self.activeSessionId = undefined;
          throw new Error(lastRetryMessage || 'OpenCode assistant turn failed without a response');
        }
