...
async function request(action: string, payload: Record<string, unknown>, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  return requestThroughMailbox({
    ...
    parse: (content) => {
      const parsed = JSON.parse(content) as { response: GatewayMailboxResponse };
      if (!parsed.response.ok) throw new Error(parsed.response.error?.message ?? 'Gateway request failed');
      return parsed.response.result;
    },
    timeoutMs,
    timeoutMessage: `${action} timed out after ${Math.round(timeoutMs / 1000)}s`,
  });
}
